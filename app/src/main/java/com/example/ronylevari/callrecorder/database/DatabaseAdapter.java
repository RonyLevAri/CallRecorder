package com.example.ronylevari.callrecorder.database;
...
public class DatabaseAdapter {
    ...
    public ArrayList<ParentRecordingItem> getParentRecords(boolean isTrashed) {
        ...
        if((res != null))
            while(res.moveToNext()) {
                ...
                parent.setStartTime(res.getInt(res.getColumnIndex(DatabaseHelper.PARENT_OPENED_ON)));
    ...
    public void insertParentToDatabase(ArrayList<DatabaseObject> parents) {
        for(DatabaseObject p : parents) {
            //insertParentToDatabase(p);
        }
    }
    ...
    public long insertParentToDatabase(ParentRecordingItem parent) {
        ...
        long id = -1;
        //ParentRecordingItem parentObj = (ParentRecordingItem) obj;
    ...
    public ArrayList<ChildRecordItem> getChildrenOf(long parent, boolean isTrashed) {
        ...
        if((res != null)) {
            while(res.moveToNext()) {
                ...
                child.setCallTime(res.getInt(res.getColumnIndex(DatabaseHelper.CHILD_RECEIVED_ON)));
    ...
    // Database SQLiteOpenHelper inner class
     private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private static final String SQL_DELETE_EASY_ENTRIES =
                "DROP TABLE IF EXISTS " + TABLE_NAME_PARENT;
        private static final String SQL_DELETE_MEDIUM_ENTRIES =
        ...
        public void initData(SQLiteDatabase db) {
            Log.d(LOG, "initiating data");
            ArrayList<ParentRecordingItem> parents = new ArrayList<>();

            for (int i = 0; i < 2; i++) {
                ParentRecordingItem parent = new ParentRecordingItem();
                parent.setStartTime(new Date().getTime());
                if (i % 2 == 0)
                    parent.setIsClosed(false);
                else
                    parent.setIsClosed(true);
                parent.setIsTrashed(false);
                parent.setRecordName("Recoring #" + i);
                parent.setNumActiveChildren(2);
                parent.setNumTrashedChildren(0);

                ContentValues contentValues = new ContentValues();
                contentValues.put(PARENT_NAME, parent.getRecordName());
                contentValues.put(PARENT_OPENED_ON, parent.getStartTime());
                if (parent.getIsClosed()) {
                    contentValues.put(PARENT_CLOSED_ON, parent.getEndTime());
                } else {
                    contentValues.putNull(PARENT_CLOSED_ON);
                }
                contentValues.put(PARENT_IS_CLOSED, parent.getIsClosed());
                contentValues.put(PARENT_IS_TRASHED, parent.getIsTrashed());
                contentValues.put(PARENT_NUM_ACTIVE_CHILDREN, parent.getNumActiveChildren());
                contentValues.put(PARENT_NUM_TRASHED_CHILDREN, parent.getNumTrashedChildren());

                long id = db.insert(TABLE_NAME_PARENT, null, contentValues);

                if (id < 0) {
                    Log.d(LOG, "Error in initializing parent");
                } else {
                    for (int j = 0; j < 2; j++) {
                        ChildRecordItem child = new ChildRecordItem();
                        child.setParent(id);
                        child.setCaller("Caller " + j);
                        child.setPhoneNumber("Caller " + j);
                        child.setCallTime(new Date().getTime());
                        child.setIsSMS(false);
                        child.setIsTrashed(false);
                        ContentValues contentValuesC = new ContentValues();
                        contentValuesC.put(CHILD_PARENT, child.getParent());
                        contentValuesC.put(CHILD_CALLER, child.getCaller());
                        contentValuesC.put(CHILD_PHONE_NUMBER, child.getPhoneNumber());
                        contentValuesC.put(CHILD_RECEIVED_ON, child.getCallTime());
                        contentValuesC.put(CHILD_IS_SMS, child.getIsSMS());
                        if (child.getIsSMS()) {
                            contentValuesC.put(CHILD_MESSAGE, child.getMessage());
                        } else {
                            contentValuesC.putNull(CHILD_MESSAGE);
                        }
                        contentValuesC.put(CHILD_IS_TRASHED, child.getIsTrashed());
                        long id2 = db.insert(TABLE_NAME_CHILDREN, null, contentValuesC);
                        if (id2 < 0) {
                            Log.d(LOG, "Error in initializing parent");
                        }
                    }
